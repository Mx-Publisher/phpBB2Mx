...
class Template 
{
	...
	*/
	...
	{
		global $cache, $user, $config, $theme; 
	...
	*/
	...
	function subtemplates_make_filename($filename)
	{
		...
		if ( isset($sub_templates) )
		{
			...
			{
				$this->theme['head_stylesheet'] = $sub_css_file;
			}
	...
	function make_filename($filename, $xs_include = false)
	{
		...
		$style_path = $this->theme['template_name'];
	...
	*/
	function compile_tag_block($tag_args)
	{
		...
		$this->block_names = is_array($this->block_names) ? $this->block_names : array($this->block_names, '');
		$tag_template_php = '';
		...
		for($i = 0; $i < $line_count; $i++)
		{
			...
			*/
			if($keyword_type == XS_TAG_INCLUDEJS)
			{
				...
				$mx_js_path =  'templates/' . rawurlencode($this->theme['template_name'] ? $this->theme['template_name'] : str_replace('.css', '', $this->theme['head_stylesheet'])) . '/';
				$asset_file = array_shift($includejs_blocks);
				$line = '<script type="text/javascript" src="' . $board_url . $mx_js_path . $asset_file . '"></script><br />';
				$compiled[] = $line;
				continue;
			}
			/*
			...
			*/
			if($keyword_type == XS_TAG_INCLUDECSS)
			{
				...
				$mx_css_path =  'templates/' . rawurlencode($this->theme['template_name'] ? $this->theme['template_name'] : str_replace('.css', '', $this->theme['head_stylesheet'])) . '/';
				$asset_file = array_shift($includecss_blocks);
				$line = '<link href="' . $board_url . $mx_css_path . $asset_file . '" rel="stylesheet" type="text/css" media="screen" />';
				$compiled[] = $line;
				continue;
			}
			/*
