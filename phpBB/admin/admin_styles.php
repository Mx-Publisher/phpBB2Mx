 ...
 *
 *   $Id: admin_styles.php,v 1.1 2010/10/10 15:05:22 orynider Exp $
 *
 *
 ***************************************************************************/
...
switch( $mode )
{
	case "addnew":
		...
		$install_to = urldecode(request_var('install_to', ''));
		$style_name = urldecode(request_var('style', ''));
		
		$cfg = array();						
		$cfg['installed'] = false;
		
		if(is_get('install_to'))
		{
			$working_data[0]['template_name'] = basename($install_to);
			$working_data[0]['style_name'] = basename($style_name);

			$template_name = basename($install_to);						
						
			// If the style author specified the theme needs to be cached
			// (because of the used paths and variables) than make sure it is the case.
			// For example, if the theme uses language-specific images it needs to be stored in db.
			if (is_file($phpbb_root_path . $user->template_path . $template_name . '/theme/stylesheet.css'))
			{
							
				//phpBB3 Style Sheet
				$theme_file = 'theme/stylesheet.css'; 
				$css_file_path = $user->template_path . $template_name . '/';
				$stylesheet = file_get_contents("{$phpbb_root_path}{$user->template_path}{$template_name}/theme/stylesheet.css");
			}
			else
			{	
				//phpBB2 Style Sheet	
				$theme_file = $template_name . '.css'; 
				$css_file_path = $user->template_path . $template_name . '/';
				if (is_file($phpbb_root_path . $user->template_path . $template_name . '/' . $theme_file))
				{
					$stylesheet = file_get_contents("{$phpbb_root_path}{$user->template_path}{$template_name}/{$theme_file}");
				}
			}
			
			if( @file_exists(@phpbb_realpath($phpbb_root_path. $user->template_path . basename($install_to) . "/theme_info.cfg")) )
			{
				include($phpbb_root_path. $user->template_path . basename($install_to) . "/theme_info.cfg");
				
				for($i = 0; $i < @count($$install_to); $i++)
				{
					$working_data = $$install_to;
								
					$style_name = $working_data[$i]['style_name'];														
				}
			}
			else if( @file_exists(@phpbb_realpath($phpbb_root_path. $user->template_path . basename($install_to) . "/style.cfg")) )
			{		
				$cfg = parse_cfg_file($phpbb_root_path. $user->template_path . basename($install_to) . '/style.cfg');
					
				//Check if is a phpBB3 style or if is a Icy-Phoenix Style
				$style_name = isset($cfg['name']) ? $cfg['name'] : basename($install_to);
							
				$working_data[0]['template_name'] = $template_name;
				$working_data[0]['style_name'] = $style_name;
				$working_data[0]['head_stylesheet'] = $theme_file;
				$working_data[0]['body_background'] = "";
				$working_data[0]['body_bgcolor'] = "";
				$working_data[0]['body_text'] = "";
				$working_data[0]['body_link'] = "";
				$working_data[0]['body_vlink'] = "";
				$working_data[0]['body_alink'] = "";
				$working_data[0]['body_hlink'] = "";
				$working_data[0]['tr_color1'] = "";
				$working_data[0]['tr_color2'] = "";
				$working_data[0]['tr_color3'] = "";
				$working_data[0]['tr_class1'] = "";
				$working_data[0]['tr_class2'] = "";
				$working_data[0]['tr_class3'] = "";
				$working_data[0]['th_color1'] = "";
				$working_data[0]['th_color2'] = "";
				$working_data[0]['th_color3'] = "";
				$working_data[0]['th_class1'] = "";
				$working_data[0]['th_class2'] = "";
				$working_data[0]['th_class3'] = "";
				$working_data[0]['td_color1'] = "";
				$working_data[0]['td_color2'] = "";
				$working_data[0]['td_color3'] = "";
				$working_data[0]['td_class1'] = "bg1";
				$working_data[0]['td_class2'] = "bg2";
				$working_data[0]['td_class3'] = "bg3";
				$working_data[0]['fontface1'] = "";
				$working_data[0]['fontface2'] = "";
				$working_data[0]['fontface3'] = "";
				$working_data[0]['fontsize1'] = "10";
				$working_data[0]['fontsize2'] = "11";
				$working_data[0]['fontsize3'] = "12";
				$working_data[0]['fontcolor1'] = "";
				$working_data[0]['fontcolor2'] = "00AA00";
				$working_data[0]['fontcolor3'] = "AA0000";
				$working_data[0]['span_class1'] = "";
				$working_data[0]['span_class2'] = "";
				$working_data[0]['span_class3'] = "";
				$working_data[0]['img_size_poll'] = "0";
				$working_data[0]['img_size_privmsg'] = "0";
			...
			$db_fields = array();
			$db_values = array();			
			for($i = 0; $i < count($working_data) && !$found; $i++)
			{
				if( $working_data[$i]['style_name'] == $style_name )
				{
					while(list($key, $val) = each($working_data[$i]))
					{
						$db_fields[] = $key;
		...
		{
			$cfg = $installed_themes = $installable_themes = array();						
			$installed = $cfg['installed'] = false;
		
			if( $dir = @opendir($phpbb_root_path. $user->template_path) )
			{
				while( $sub_dir = @readdir($dir) )
				{
					if( !is_file(phpbb_realpath($phpbb_root_path . $user->template_path .$sub_dir)) && !is_link(phpbb_realpath($phpbb_root_path . $user->template_path .$sub_dir)) && $sub_dir != "." && $sub_dir != ".." && $sub_dir != "CVS" )
					{
						$template_name = basename($sub_dir);						
						...
						if( @file_exists(@phpbb_realpath($phpbb_root_path. $user->template_path . $sub_dir . "/theme_info.cfg")) )
						{
							include($phpbb_root_path. $user->template_path . $sub_dir . "/theme_info.cfg");
						...
						else if( @file_exists(@phpbb_realpath($phpbb_root_path. $user->template_path . $sub_dir . "/style.cfg")) )
						{		
							$cfg = parse_cfg_file($phpbb_root_path. $user->template_path . $sub_dir . '/style.cfg');
							
							$cfg['installed'] = $installed;							
							
							//Check if is a phpBB3 style or if is a Icy-Phoenix Style
							$style_name = isset($cfg['name']) ? $cfg['name'] : basename($sub_dir);
						...
						for($i = 0; $i < $count = count($sub_dir); $i++)
						{																	
							//BaseName StyleName									
							$style_name = $working_data[$i]['style_name'] = basename(str_replace("\'", "''", $style_name));
							...
							if(!$db->sql_numrows($result))
							{
								...
								$cfg['installed'] = $installed = true;
							}
							else
							{
								$installed_themes[] = $working_data[$i];
								
								$cfg['installed'] = $installed = false;
							}														
						}		
					}
				}
	...
	case "edit":
		...
		else
		{
			...
			else
			{
				...
				/** **/
				if( empty($selected['template_name']) )
				{
						
						//	
						// DAMN! Thats alot of data to validate...
						//
						$selected['style_name'] = 'prosilver4';
						$selected['template_name'] = 'prosilver4';
						$selected['head_stylesheet'] = 'theme/stylesheet.css';
						$selected['body_background'] = '';
						$selected['body_bgcolor'] = '';
						$selected['body_text'] = '';
						$selected['body_link'] = '';
						$selected['body_vlink'] = '';
						$selected['body_alink'] = '';
						$selected['body_hlink'] = '';
						$selected['tr_color1'] = '';
						$selected_name['tr_color1_name'] =  '';
						$selected['tr_color2'] = '';
						$selected_name['tr_color2_name'] = '';
						$selected['tr_color3'] = '';
						$selected_name['tr_color3_name'] = '';
						$selected['tr_class1'] = '';
						$selected_name['tr_class1_name'] = '';
						$selected['tr_class2'] = '';
						$selected_name['tr_class2_name'] = '';
						$selected['tr_class3'] = '';
						$selected_name['tr_class3_name'] = '';
						$selected['th_color1'] = '';
						$selected_name['th_color1_name'] = '';
						$selected['th_color2'] = '';
						$selected_name['th_color2_name'] = '';
						$selected['th_color3'] = '';
						$selected_name['th_color3_name'] = '';
						$selected['th_class1'] = '';
						$selected_name['th_class1_name'] = '';
						$selected['th_class2'] = '';
						$selected_name['th_class2_name'] = '';
						$selected['th_class3'] = '';
						$selected_name['th_class3_name'] = '';
						$selected['td_color1'] = '';
						$selected_name['td_color1_name'] = '';
						$selected['td_color2'] = '';
						$selected_name['td_color2_name'] = '';
						$selected['td_color3'] = '';
						$selected_name['td_color3_name'] = '';
						$selected['td_class1'] = '';
						$selected_name['td_class1_name'] = '';
						$selected['td_class2'] = '';
						$selected_name['td_class2_name'] = '';
						$selected['td_class3'] = '';
						$selected_name['td_class3_name'] = '';
						$selected['fontface1'] = '';
						$selected_name['fontface1_name'] = '';
						$selected['fontface2'] = '';
						$selected_name['fontface2_name'] = '';
						$selected['fontface3'] = '';
						$selected_name['fontface3_name'] = '';
						$selected['fontsize1'] = '';
						$selected_name['fontsize1_name'] = '';
						$selected['fontsize2'] = '';
						$selected_name['fontsize2_name'] = '';
						$selected['fontsize3'] = '';
						$selected_name['fontsize3_name'] = '';
						$selected['fontcolor1'] = '';
						$selected_name['fontcolor1_name'] = '';
						$selected['fontcolor2'] = '';
						$selected_name['fontcolor2_name'] = '';
						$selected['fontcolor3'] = '';
						$selected_name['fontcolor3_name'] = '';
						$selected['span_class1'] = '';
						$selected_name['span_class1_name'] = '';
						$selected['span_class2'] = '';
						$selected_name['span_class2_name'] = '';
						$selected['span_class3'] = '';
						$selected_name['span_class3_name'] = '';
						//
						// Wheeeew! Thank heavens for copy and paste and search and replace :D
						//							
			...
			{	
				...
				{	
					if( !is_file(phpbb_realpath($phpbb_root_path . $user->template_path . $file)) && !is_link(phpbb_realpath($phpbb_root_path . $user->template_path . $file)) && $file != "." && $file != ".." && $file != "CVS" )
					{
			...
			$template->assign_vars(array(
				...
				"TR_COLOR1_NAME" => empty($selected['tr_color1_name']) ? '' : $selected['tr_color1_name'],
				"TR_COLOR2_NAME" => empty($selected['tr_color2_name']) ? '' : $selected['tr_color2_name'],
				"TR_COLOR3_NAME" => empty($selected['tr_color3_name']) ? '' : $selected['tr_color3_name'],
				"TR_CLASS1_NAME" => empty($selected['tr_class1_name']) ? '' : $selected['tr_class1_name'],
				"TR_CLASS2_NAME" => empty($selected['tr_class2_name']) ? '' : $selected['tr_class2_name'],
				"TR_CLASS3_NAME" => empty($selected['tr_class3_name']) ? '' : $selected['tr_class3_name'],
				"TH_COLOR1_NAME" => empty($selected['th_color1_name']) ? '' : $selected['th_color1_name'],
				"TH_COLOR2_NAME" => empty($selected['th_color2_name']) ? '' : $selected['th_color2_name'],
				"TH_COLOR3_NAME" => empty($selected['th_color3_name']) ? '' : $selected['th_color3_name'],
				"TH_CLASS1_NAME" => empty($selected['th_class1_name']) ? '' : $selected['th_class1_name'],
				"TH_CLASS2_NAME" => empty($selected['th_class2_name']) ? '' : $selected['th_class2_name'],
				"TH_CLASS3_NAME" => empty($selected['th_class3_name']) ? '' : $selected['th_class3_name'],
				"TD_COLOR1_NAME" => empty($selected['td_color1_name']) ? '' : $selected['td_color1_name'],
				"TD_COLOR2_NAME" => empty($selected['td_color2_name']) ? '' : $selected['td_color2_name'],
				"TD_COLOR3_NAME" => empty($selected['td_color3_name']) ? '' : $selected['td_color3_name'],
				"TD_CLASS1_NAME" => empty($selected['td_class1_name']) ? '' : $selected['td_class1_name'],
				"TD_CLASS2_NAME" => empty($selected['td_class2_name']) ? '' : $selected['td_class2_name'],
				"TD_CLASS3_NAME" => empty($selected['td_class3_name']) ? '' : $selected['td_class3_name'],
				"FONTFACE1_NAME" => empty($selected['fontface1_name']) ? '' : $selected['fontface1_name'],
				"FONTFACE2_NAME" => empty($selected['fontface2_name']) ? '' : $selected['fontface2_name'],
				"FONTFACE3_NAME" => empty($selected['fontface3_name']) ? '' : $selected['fontface3_name'],
				"FONTSIZE1_NAME" => empty($selected['fontsize1_name']) ? '' : $selected['fontsize1_name'],
				"FONTSIZE2_NAME" => empty($selected['fontsize2_name']) ? '' : $selected['fontsize2_name'],
				"FONTSIZE3_NAME" => empty($selected['fontsize3_name']) ? '' : $selected['fontsize3_name'],
				"FONTCOLOR1_NAME" => empty($selected['fontcolor1_name']) ? '' : $selected['fontcolor1_name'],
				"FONTCOLOR2_NAME" => empty($selected['fontcolor2_name']) ? '' : $selected['fontcolor2_name'],
				"FONTCOLOR3_NAME" => empty($selected['fontcolor3_name']) ? '' : $selected['fontcolor3_name'],
				"SPAN_CLASS1_NAME" => empty($selected['span_class1_name']) ? '' : $selected['span_class1_name'],
				"SPAN_CLASS2_NAME" => empty($selected['span_class2_name']) ? '' : $selected['span_class2_name'],
				"SPAN_CLASS3_NAME" => empty($selected['span_class3_name']) ? '' : $selected['span_class3_name'],
	...
	case "export";
		if(is_post('export_template'))
		{
			$template_name = request_var('export_template', 'prosilver2');
			...
			$fp = @fopen($phpbb_root_path . $user->template_path . basename($template_name) . '/theme_info.cfg', 'w');
	...
	case "delete":
		...
		else
		{
			...
			//DELETE FROM phpbb_themes_name WHERE themes_id = 9
			$sql = "DELETE FROM " . THEMES_NAME_TABLE . " 
				WHERE themes_id = $style_id";
			//$db->sql_query($sql);
			if(!@$db->sql_query($sql))
			{
				message_die(GENERAL_ERROR, "Could not DELETE style information", "", __LINE__, __FILE__, $sql);
			}
			...
			//DELETE FROM `phpbb_themes` WHERE `phpbb_themes`.`themes_id` = 19 
			$sql = "DELETE FROM `" . THEMES_TABLE . "` 
				WHERE `" . THEMES_TABLE . "`.`themes_id` = $style_id";
			...
			message_die(GENERAL_MESSAGE, $message, "", __LINE__, __FILE__, print_r($sql, false));
		}
		break;
